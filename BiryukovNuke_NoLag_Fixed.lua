...
-- ── NOTIFICATION SYSTEM (Optimized - no lag) ──
local _notifGui = nil
local _notifQueue = {}
...
local function _getNotifGui()
    ...
    _notifGui.Name = "_FazoxNotifGui"
    _notifGui.ResetOnSpawn = false
    _notifGui.DisplayOrder = 12010
    _notifGui.Parent = playerGui
    return _notifGui
end

local function _cleanupNotif(frame)
    pcall(function()
...
local function showNotif(opts)
    local sg = _getNotifGui()

    -- Limit concurrent notifications
    if _notifActive >= MAX_NOTIFS then
        -- Remove oldest notification
        for _, child in ipairs(sg:GetChildren()) do
            if child.Name == "NotifWrap" then
                _cleanupNotif(child)
                break
            end
        end
    ...
    wrap.Position = UDim2.new(0.5, 0, 1.15, 0)
    wrap.Size = UDim2.fromOffset(280, 52)
    ...
    pill.BackgroundColor3 = opts.Color or Color3.fromRGB(30, 30, 40)
    ...
    -- Animate in (simple, no Back easing to reduce computation)
    pill.BackgroundTransparency = 0
    lbl.TextTransparency = 0
    wrap.Position = UDim2.new(0.5, 0, 0.88, 0)
    ...
    task.delay(duration, function()
        ...
        TweenService:Create(pill, fadeOut, {BackgroundTransparency = 1}):Play()
        TweenService:Create(lbl, fadeOut, {TextTransparency = 1}):Play()
...
local function showGreenDonation(amount, recipientName)
    ...
    wrap.AnchorPoint = Vector2.new(0.5, 1)
    wrap.Position = UDim2.new(0.5, 0, 1.15, 0)
    wrap.Size = UDim2.fromOffset(320, 58)
    wrap.BackgroundTransparency = 1
    wrap.BorderSizePixel = 0
    wrap.Parent = sg
    ...
    pill.AnchorPoint = Vector2.new(0.5, 0.5)
    pill.Position = UDim2.new(0.5, 0, 0.5, 0)
    pill.BackgroundColor3 = Color3.fromRGB(0, 140, 60)
    pill.BorderSizePixel = 0
    pill.BackgroundTransparency = 1
    pill.Parent = wrap
    Instance.new("UICorner", pill).CornerRadius = UDim.new(1, 0)
    ...
    local ico = Instance.new("ImageLabel", pill)
    ico.Image = ROBUX_ICON
    ico.Size = UDim2.fromOffset(24, 24)
    ico.BackgroundTransparency = 1
    ico.ImageColor3 = Color3.fromRGB(180, 255, 180)
    ico.LayoutOrder = 1

    local txt = Instance.new("TextLabel", pill)
    txt.Text = "you donated " .. fmt(amount) .. " to " .. tostring(recipientName) .. "!"
    ...
    -- Instant show (no tween for position to reduce lag)
    pill.BackgroundTransparency = 0
    txt.TextTransparency = 0
    wrap.Position = UDim2.new(0.5, 0, 0.82, 0)

    task.delay(4, function()
        if not (wrap and wrap.Parent) then return end
        local fadeOut = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        TweenService:Create(pill, fadeOut, {BackgroundTransparency = 1}):Play()
        TweenService:Create(txt, fadeOut, {TextTransparency = 1}):Play()
        task.delay(0.35, function()
            _cleanupNotif(wrap)
        end)
    end)
...
local function showSentGiftNotification()
    ...
    wrap.Name = "NotifWrap"
    wrap.AnchorPoint = Vector2.new(0.5, 1)
    wrap.Position = UDim2.new(0.5, 0, 1.2, 0)
    wrap.Size = UDim2.fromOffset(280, 56)
    wrap.BackgroundTransparency = 1
    wrap.BorderSizePixel = 0
    wrap.Parent = sg
    ...
    container.BackgroundColor3 = Color3.fromRGB(255, 20, 110)
    ...
    local sentWord = Instance.new("TextLabel", container)
    sentWord.Text = "sent"
    sentWord.Font = Enum.Font.GothamBlack
    sentWord.TextSize = 20
    sentWord.TextColor3 = Color3.new(1, 1, 1)
    sentWord.BackgroundTransparency = 1
    sentWord.Size = UDim2.new(0, 0, 1, 0)
    sentWord.AutomaticSize = Enum.AutomaticSize.X
    sentWord.LayoutOrder = 1

    local giftWord = Instance.new("TextLabel", container)
    giftWord.Text = "gift!"
    giftWord.Font = Enum.Font.GothamBlack
    giftWord.TextSize = 20
    giftWord.TextColor3 = Color3.fromRGB(255, 230, 60)
    giftWord.BackgroundTransparency = 1
    giftWord.Size = UDim2.new(0, 0, 1, 0)
    giftWord.AutomaticSize = Enum.AutomaticSize.X
    giftWord.LayoutOrder = 2
    ...
    cStroke.Transparency = 0.3
    sentWord.TextTransparency = 0
    giftWord.TextTransparency = 0
    wrap.Position = UDim2.new(0.5, 0, 0.88, 0)
    ...
    task.delay(3.5, function()
        ...
        TweenService:Create(cStroke, fadeOut, {Transparency = 1}):Play()
        TweenService:Create(sentWord, fadeOut, {TextTransparency = 1}):Play()
        TweenService:Create(giftWord, fadeOut, {TextTransparency = 1}):Play()
...
local function fireNotifs(recipientName, amount, style)
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Name:lower() == (recipientName or ""):lower() then
            online = true
            break
        end
    end
    ...
    if style == "pink" then
        task.delay(0.3, function() showSentGiftNotification() end)
    elseif style == "green" then
        showGreenDonation(amount, recipientName)
    else
        if online then
            showGreenDonation(amount, recipientName)
        else
...
local function triggerNuke(recipientName, amount)
    ...
    task.spawn(function()
        ...
        local bp=getBasePart(); if not bp then pcall(function() clone:Destroy() end); pcall(function() if doRestoreNuke then doRestoreNuke() end end); _nukeActive=false; return end
...
local function openKratoBuy(data)
    ...
    kBuyFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    kSuccessFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
...
kOkBtn.MouseButton1Click:Connect(function()
    ...
    kSuccessFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    kBuyFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    kDeb=false; CFG.isMenuOpen=false
end)
...
-- Make panel draggable (mobile + desktop) — Active flag required for Touch on a Frame
panel.Active = true
...
do
    ...
    UserInputService.InputChanged:Connect(function(inp)
        ...
        if inp ~= activeInput and inp.UserInputType ~= Enum.UserInputType.MouseMovement then return end
        if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then
            local delta = inp.Position - dragStart
            panel.Position = UDim2.new(frameStart.X.Scale, frameStart.X.Offset + delta.X, frameStart.Y.Scale, frameStart.Y.Offset + delta.Y)
        end
    end)
    UserInputService.InputEnded:Connect(function(inp)
        if inp == activeInput or inp.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = false
            activeInput = nil
        end
    end)
...
closeHubBtn.MouseButton1Click:Connect(function() pcall(function() if destroyOverlay then destroyOverlay() end end); pcall(function() if setHide then setHide(false) end end); pcall(function() mainGui:Destroy() end) end)
